...
Deno.serve(async (request) => {
  ...
  const { data: organizer, error: organizerError } = await admin.from("organizers").select("id").eq("id", authData.user.id).maybeSingle();
  if (organizerError) {
  ...
  try {
    if (action === "bootstrap") {
      ...
      return respond(200, {
        organizer: { id: authData.user.id, email: authData.user.email || "Organizer" },
    ...
    if (action === "applicationDetail") {
      ...
      if (error) throw error;
      return respond(200, { application: data }, origin);
    }
    ...
    if (action === "updateApplication") {
      ...
      if (error?.code === "PGRST116") return respond(404, { error: "Application not found." }, origin);
      if (error) throw error;
      return respond(200, { application: data, message: "Application updated." }, origin);
    }
    ...
    if (action === "exportApplications") {
      ...
      if (error) throw error;
      return respond(200, { applications: data || [] }, origin);
    }
    ...
    if (action.toLowerCase().includes("organizer")) return respond(403, { error: "Organizer access cannot be changed here." }, origin);
